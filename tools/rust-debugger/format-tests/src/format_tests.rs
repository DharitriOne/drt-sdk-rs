use drt_sc::{
    ...
    types::{
        ...
        BigFloat, BigInt, BigUint, DCTSystemSCAddress, RewaOrDctTokenIdentifier,
        DctTokenPayment, ManagedAddress, ManagedBuffer, ManagedByteArray, ManagedOption,
...
fn main() {
    ...
    let managed_address = DCTSystemSCAddress.to_managed_address::<DebugApi>();
    push!(
        to_check,
        managed_address,
    ...
    let rewa_or_dct_token_identifier_rewa: RewaOrDctTokenIdentifier<DebugApi> =
        RewaOrDctTokenIdentifier::rewa();
    push!(
        to_check,
        rewa_or_dct_token_identifier_rewa,
        "RewaOrDctTokenIdentifier::rewa()"
    );

    let rewa_or_dct_token_identifier_dct: RewaOrDctTokenIdentifier<DebugApi> =
        RewaOrDctTokenIdentifier::dct("MYTOK-123456");
    push!(
        to_check,
        rewa_or_dct_token_identifier_dct,
        "RewaOrDctTokenIdentifier::dct(\"MYTOK-123456\")"
    );
