...
DEBUG_API_TYPE = "drt_sc_scenario::api::impl_vh::vm_hooks_api::VMHooksApi<drt_sc_scenario::api::impl_vh::debug_api::DebugApiBackend>"
ANY_NUMBER = "[0-9]+"
ANY_TYPE = ".*"
...
REWA_OR_DCT_TOKEN_IDENTIFIER_TYPE = f"{MOD_PATH}::rewa_or_dct_token_identifier::RewaOrDctTokenIdentifier<{DEBUG_API_TYPE}>"
...
# 6. DharitriOne codec - Multi-types
...
class RewaOrDctTokenIdentifier(PlainManagedVecItem, ManagedType):
    def lookup(self, rewa_or_dct_token_identifier: lldb.value) -> lldb.value:
        return rewa_or_dct_token_identifier.data

    @check_invalid_handle
    def summary_from_raw_handle(self, raw_handle: int, context: lldb.value, type_info: lldb.SBType) -> str:
        if raw_handle == MANAGED_OPTION_NONE_HANDLE:
            return "RewaOrDctTokenIdentifier::rewa()"
        token_summary = TokenIdentifier().summary_from_raw_handle(raw_handle, context, None)
        return f"RewaOrDctTokenIdentifier::dct({token_summary})"
...
NUMBAT_WASM_TYPE_HANDLERS = [
    ...
    (REWA_OR_DCT_TOKEN_IDENTIFIER_TYPE, RewaOrDctTokenIdentifier),
