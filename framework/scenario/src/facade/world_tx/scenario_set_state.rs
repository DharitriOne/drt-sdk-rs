...
use drt_chain_vm::world_mock::DctInstanceMetadata;
use drt_sc::{
    proxy_imports::TopEncode,
    types::{AnnotatedValue, BigUint, DctLocalRole, ManagedAddress},
};
...
impl ScenarioWorld {
    ...
    pub fn set_dct_balance<A, V>(&mut self, address: A, token_id: &[u8], balance: V)
    where
        A: AnnotatedValue<ScenarioTxEnvData, ManagedAddress<StaticApi>>,
        V: AnnotatedValue<ScenarioTxEnvData, BigUint<StaticApi>>,
    {
        ...
        C: AnnotatedValue<ScenarioTxEnvData, ManagedAddress<StaticApi>>,
    {
        ...
        let mut dct_attributes = Vec::new();
        let _ = attributes.top_encode(&mut dct_attributes);
        let accounts = &mut self.get_mut_state().accounts;
        for (vm_address, account) in accounts.iter_mut() {
            if vm_address == &address_value.to_vm_address() {
                account.dct.set_dct_balance(
                    ...
                    DctInstanceMetadata {
                        ...
                        attributes: dct_attributes.clone(),
    ...
    pub fn set_dct_local_roles<A>(&mut self, address: A, token_id: &[u8], roles: &[DctLocalRole])
    where
        A: AnnotatedValue<ScenarioTxEnvData, ManagedAddress<StaticApi>>,
    {
        let env = self.new_env_data();
