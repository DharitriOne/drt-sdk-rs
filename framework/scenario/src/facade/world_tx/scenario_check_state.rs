...
use drt_chain_scenario_format::interpret_trait::{InterpretableFrom, InterpreterContext};
use drt_sc::{
...
use crate::{
    ...
    scenario_model::{
        AddressKey, BytesKey, BytesValue, CheckAccount, CheckDct, CheckDctData,
        CheckDctInstances, CheckDctMap, CheckDctMapContents, CheckStateStep, CheckStorage,
...
impl<'w> CheckStateBuilder<'w> {
    ...
    pub fn dct_balance<K, V>(mut self, token_id: K, balance: V) -> Self
    where
        ...
        V: AnnotatedValue<ScenarioTxEnvData, BigUint<StaticApi>>,
    {
        ...
        match &mut self.current_account.dct {
            CheckDctMap::Unspecified | CheckDctMap::Star => {
                let mut new_dct_map = BTreeMap::new();
                let _ = new_dct_map.insert(token_id_key, CheckDct::Short(balance_value));
                ...
                self.current_account.dct = CheckDctMap::Equal(new_check_dct_map);
            },
            CheckDctMap::Equal(check_dct_map) => {
                if check_dct_map.contents.contains_key(&token_id_key) {
                    let prev_entry = check_dct_map.contents.get_mut(&token_id_key).unwrap();
                    match prev_entry {
                        CheckDct::Short(prev_balance_check) => *prev_balance_check = balance_value,
        ...
        T: TopEncode,
    {
        ...
        if let CheckDctMap::Unspecified = &self.current_account.dct {
            ...
            let mut new_dct_map = BTreeMap::new();
            let _ = new_dct_map.insert(token_id_key, check_dct);
