...
use crate::{
    ...
    scenario_model::{Account, AddressKey, BytesKey, Dct, DctObject, SetStateStep},
    ScenarioTxEnvData,
};
...
impl<'w> SetStateBuilder<'w, AccountItem> {
        ...
        V: AnnotatedValue<ScenarioTxEnvData, BigUint<StaticApi>>,
    {
        ...
        let dct_data_ref = self.get_dct_data_or_create(&token_id_key);
        dct_data_ref.set_balance(0u64, balance_value);
        ...
        T: AnnotatedValue<ScenarioTxEnvData, ManagedBuffer<StaticApi>>,
    {
        ...
        let dct_obj_ref = self
            .get_dct_data_or_create(&token_id_key)
            .get_mut_dct_object();
        dct_obj_ref.set_balance(nonce_value.clone(), balance_value);

        dct_obj_ref.set_token_attributes(nonce_value, attributes_value);

        self
    }

    #[allow(clippy::too_many_arguments)]
    pub fn dct_nft_all_properties<K, N, V, T, C, R, H, U>(
        mut self,
        token_id: K,
        nonce: N,
        balance: V,
    ...
    pub fn dct_nft_last_nonce<K, N>(mut self, token_id: K, last_nonce: N) -> Self
    where
        K: AnnotatedValue<ScenarioTxEnvData, TokenIdentifier<StaticApi>>,
        N: AnnotatedValue<ScenarioTxEnvData, u64>,
    {
        ...
        let dct_obj_ref = self
            .get_dct_data_or_create(&token_id_key)
            .get_mut_dct_object();
        dct_obj_ref.set_last_nonce(nonce_value);

        self
    }

    // TODO: Find a better way to pass roles
    pub fn dct_roles<K>(mut self, token_id: K, roles: Vec<String>) -> Self
    where
        K: AnnotatedValue<ScenarioTxEnvData, TokenIdentifier<StaticApi>>,
    {
        let env = self.new_env_data();
    ...
    fn get_dct_data_or_create(&mut self, token_id: &BytesKey) -> &mut Dct {
        if !self.item.account.dct.contains_key(token_id) {
            self.item
                .account
                .dct
        ...
        self.item.account.dct.get_mut(token_id).unwrap()
    }
