use drt_sc::{api::ManagedTypeApi, types::DctTokenPayment};
...
impl<M: ManagedTypeApi> From<DctTokenPayment<M>> for TxDCT {
    fn from(value: DctTokenPayment<M>) -> Self {
        TxDCT {
            dct_token_identifier: BytesValue::from(
                value.token_identifier.as_managed_buffer().to_vec(),
            ),
            nonce: U64Value::from(value.token_nonce),
            dct_value: BigUintValue::from(value.amount),
        }
    }
}
