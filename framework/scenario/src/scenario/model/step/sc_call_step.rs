use drt_sc::{abi::TypeAbiFrom, types::H256};
...
impl ScCallStep {
    ...
    where
        BigUintValue: From<A>,
    {
        if !self.tx.dct_value.is_empty() && self.tx.rewa_value.value > 0u32.into() {
            panic!("Cannot transfer both REWA and DCT");
        }
    ...
    where
        CC: drt_sc::types::ContractCallBase<StaticApi>,
    {
