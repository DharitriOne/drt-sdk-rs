use drt_sc::{
    ...
    types::{
        BigUint, DctTokenPayment, ManagedByteArray, ManagedType, ManagedVecItemPayload,
        TokenIdentifier,
    },
};
...
pub struct ManagedStructWithToken<M: ManagedTypeApi> {
    pub token: drt_sc::types::DctTokenPayment<M>,
    pub num: u32,
...
fn struct_with_numbers_static() {
    assert_eq!(
        <ManagedStructWithToken<StaticApi> as drt_sc::types::ManagedVecItem>::payload_size(),
        28
    );
    assert!(
        !<ManagedStructWithToken<StaticApi> as drt_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn struct_to_bytes_writer() {
    ...
    let mut payload = <ManagedStructWithToken<StaticApi> as drt_sc::types::ManagedVecItem>::PAYLOAD::new_buffer();
    ...
    <ManagedStructWithToken<StaticApi> as drt_sc::types::ManagedVecItem>::to_byte_writer(
        &s,
        |bytes| {
...
fn struct_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <ManagedStructWithToken<StaticApi> as drt_sc::types::ManagedVecItem>::from_byte_reader(
            |bytes| {
