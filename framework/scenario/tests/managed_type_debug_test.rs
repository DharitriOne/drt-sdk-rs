use drt_sc::{
    hex_literal::hex,
    types::{
        BigInt, BigUint, RewaOrDctTokenIdentifier, ManagedAddress, ManagedBuffer,
...
#[test]
fn test_managed_vec_format_rewa_or_dct() {
    let mut mv = ManagedVec::<StaticApi, RewaOrDctTokenIdentifier<StaticApi>>::new();
    mv.push(RewaOrDctTokenIdentifier::rewa());
    mv.push(RewaOrDctTokenIdentifier::dct(TokenIdentifier::from(
        "MYTOKEN-5678",
    )));
    let s = format!("{:?}", &mv);
    assert_eq!(
        "[RewaOrDctTokenIdentifier::Rewa, RewaOrDctTokenIdentifier::Dct(\"MYTOKEN-5678\")]",
        s
    );
}
