...
fn struct_with_numbers_static() {
    assert_eq!(
        <ManagedStructWithBigUint<StaticApi> as drt_sc::types::ManagedVecItem>::payload_size(
        ),
        8
    );
    assert!(
        !<ManagedStructWithBigUint<StaticApi> as drt_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn managed_struct_to_bytes_writer() {
    ...
    let mut payload = <ManagedStructWithBigUint<StaticApi> as drt_sc::types::ManagedVecItem>::PAYLOAD::new_buffer();
    ...
    <ManagedStructWithBigUint<StaticApi> as drt_sc::types::ManagedVecItem>::to_byte_writer(
        &s,
        |bytes| {
...
fn managed_struct_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <ManagedStructWithBigUint<StaticApi> as drt_sc::types::ManagedVecItem>::from_byte_reader(
            |bytes| {
