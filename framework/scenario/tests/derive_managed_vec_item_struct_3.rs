...
pub struct Auction<M: ManagedTypeApi> {
    ...
    pub payment_token_type: RewaOrDctTokenIdentifier<M>,
...
fn struct_3_static() {
    assert_eq!(
        <Auction<StaticApi> as drt_sc::types::ManagedVecItem>::payload_size(),
        74
    );
    assert!(!<Auction<StaticApi> as drt_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION);
}
