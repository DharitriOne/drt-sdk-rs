use drt_sc_scenario::{
    imports::{Address, DCTSystemSCAddress},
    drt_chain_vm::crypto_functions::keccak256,
...
fn process_new_issued_token_identifier(tx: &TransactionOnNetwork) -> Option<String> {
    for scr in tx.smart_contract_results.iter() {
        if scr.sender.to_bech32_string().unwrap() != DCTSystemSCAddress.to_bech32_string() {
            continue;
        }
        ...
        let is_register_and_set_all_roles_dct =
        ...
        if !is_issue_fungible
            ...
            && !is_register_and_set_all_roles_dct
        {
            continue;
        }
