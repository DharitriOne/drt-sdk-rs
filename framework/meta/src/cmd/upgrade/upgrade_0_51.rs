...
use drt_sc_meta_lib::cargo_toml_contents::CargoTomlContents;
use ruplacer::Query;
use std::path::Path;
use toml::Value;
...
fn v_0_51_prepare_meta(sc_crate_path: &Path) {
    ...
    print_cargo_dep_remove(cargo_toml_path.as_path(), "drt-sc-meta");
    let mut meta_value = deps
        .remove("drt-sc-meta")
        .expect("drt-sc-meta dependency not found in meta crate");
    ...
    deps.insert("drt-sc-meta-lib".to_string(), meta_value);
