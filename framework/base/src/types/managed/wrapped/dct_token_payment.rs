...
/// The version of `DctTokenPayment` that contains referrences instead of owned fields.
pub struct DctTokenPaymentRefs<'a, M: ManagedTypeApi> {
...
impl<M: ManagedTypeApi> DctTokenPayment<M> {
    pub fn as_refs(&self) -> DctTokenPaymentRefs<'_, M> {
        DctTokenPaymentRefs::new(&self.token_identifier, self.token_nonce, &self.amount)
    }
}

impl<'a, M: ManagedTypeApi> DctTokenPaymentRefs<'a, M> {
    #[inline]
    pub fn new(
    ...
    pub fn to_owned_payment(&self) -> DctTokenPayment<M> {
        DctTokenPayment {
...
impl<M: ManagedTypeApi> From<()> for MultiDctPayment<M> {
    #[inline]
    fn from(_value: ()) -> Self {
        MultiDctPayment::new()
    }
...
impl<M: ManagedTypeApi> From<DctTokenPayment<M>> for MultiDctPayment<M> {
    #[inline]
    fn from(value: DctTokenPayment<M>) -> Self {
        MultiDctPayment::from_single_item(value)
    }
}

impl<M: ManagedTypeApi> From<(TokenIdentifier<M>, u64, BigUint<M>)> for MultiDctPayment<M> {
    #[inline]
    fn from(value: (TokenIdentifier<M>, u64, BigUint<M>)) -> Self {
        MultiDctPayment::from_single_item(value.into())
    }
}
