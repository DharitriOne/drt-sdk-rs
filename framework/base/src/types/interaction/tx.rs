use crate::{
    api::CallTypeApi,
    types::{
        heap::H256, BigUint, CodeMetadata, RewaOrDctTokenIdentifier, RewaOrDctTokenPayment,
        RewaOrDctTokenPaymentRefs, RewaOrMultiDctPayment, DctTokenPayment, DctTokenPaymentRefs,
        ManagedAddress, ManagedBuffer, ManagedOption, ManagedVec, MultiDctPayment,
        TokenIdentifier,
    },
};

use drt_sc_codec::TopEncodeMulti;
...
where
    ...
    RH: TxResultHandler<Env>,
{
    /// Converts object to a DharitriOne transaction data field string.
    ...
    RH: TxResultHandler<Env>,
{
    ...
    /// Adds the first single, owned DCT token payment to a transaction.
    ///
    /// Since this is the first DCT payment, a single payment tx is produced.
    ///
    /// Can subsequently be called again for multiple payments.
    pub fn dct<P: Into<DctTokenPayment<Env::Api>>>(
        self,
        payment: P,
    ) -> Tx<Env, From, To, DctTokenPayment<Env::Api>, Gas, Data, RH> {
        self.payment(payment.into())
    }

    /// Sets a single token payment, with the token identifier and amount kept as references.
    ///
    /// This is handy whem we only want one DCT transfer and we want to avoid unnecessary object clones.
    pub fn single_dct<'a>(
        self,
        token_identifier: &'a TokenIdentifier<Env::Api>,
        token_nonce: u64,
        amount: &'a BigUint<Env::Api>,
    ) -> Tx<Env, From, To, DctTokenPaymentRefs<'a, Env::Api>, Gas, Data, RH> {
        self.payment(DctTokenPaymentRefs {
            token_identifier,
            token_nonce,
            amount,
        })
    ...
    /// Syntactic sugar for `self.payment(RewaOrDctTokenPaymentRefs::new(...)`. Takes references.
    pub fn rewa_or_single_dct<'a>(
        self,
        token_identifier: &'a RewaOrDctTokenIdentifier<Env::Api>,
        token_nonce: u64,
        amount: &'a BigUint<Env::Api>,
    ) -> Tx<Env, From, To, RewaOrDctTokenPaymentRefs<'a, Env::Api>, Gas, Data, RH> {
        self.payment(RewaOrDctTokenPaymentRefs::new(
            token_identifier,
            token_nonce,
            amount,
        ))
    ...
    /// Sets a collection of DCT transfers as the payment of the transaction.
    ///
    /// Can be formed from single DCT payments, but the result will always be a collection.
    ///
    /// Always converts the argument into an owned collection of DCT payments. For work with references, use `.payment(&p)` instead.
    pub fn multi_dct<IntoMulti>(
        self,
        payments: IntoMulti,
    ) -> Tx<Env, From, To, MultiDctPayment<Env::Api>, Gas, Data, RH>
    where
        IntoMulti: Into<MultiDctPayment<Env::Api>>,
    {
        self.payment(payments.into())
    }

    /// Backwards compatibility.
    pub fn with_dct_transfer<P: Into<DctTokenPayment<Env::Api>>>(
        self,
        payment: P,
    ) -> Tx<Env, From, To, MultiDctPayment<Env::Api>, Gas, Data, RH> {
        self.payment(MultiDctPayment::new())
    ...
    pub fn with_multi_token_transfer(
        self,
        payments: MultiDctPayment<Env::Api>,
    ) -> Tx<Env, From, To, MultiDctPayment<Env::Api>, Gas, Data, RH> {
        self.multi_dct(payments)
    }

    /// Backwards compatibility.
    pub fn with_rewa_or_single_dct_transfer<P: Into<RewaOrDctTokenPayment<Env::Api>>>(
        self,
        payment: P,
    ) -> Tx<Env, From, To, RewaOrDctTokenPayment<Env::Api>, Gas, Data, RH> {
        self.payment(payment.into())
    }

    /// Converts argument to `RewaOrMultiDctPayment`, then sets it as payment.
    ///
    /// In most cases, `payment` should be used instead.
    pub fn rewa_or_multi_dct<P: Into<RewaOrMultiDctPayment<Env::Api>>>(
        self,
        payment: P,
    ) -> Tx<Env, From, To, RewaOrMultiDctPayment<Env::Api>, Gas, Data, RH> {
        self.payment(payment.into())
    }
}

impl<Env, From, To, Gas, Data, RH> Tx<Env, From, To, DctTokenPayment<Env::Api>, Gas, Data, RH>
where
    Env: TxEnv,
    From: TxFrom<Env>,
    To: TxTo<Env>,
    ...
    RH: TxResultHandler<Env>,
{
    /// Adds the second DCT token transfer to a contract call.
    ///
    /// Can be called multiple times on the same call.
    ///
    /// When the Tx already contains a single (owned) DCT payment,
...
impl<Env, From, To, Gas, Data, RH> Tx<Env, From, To, MultiDctPayment<Env::Api>, Gas, Data, RH>
where
    Env: TxEnv,
    From: TxFrom<Env>,
    To: TxTo<Env>,
    ...
    RH: TxResultHandler<Env>,
{
    ...
    /// When the Tx already contains an owned collection of DCT payments,
    /// calling `multi_dct` is equivalent to `dct`, it just adds another payment to the list.
    ///
    /// Can be called multiple times.
    pub fn multi_dct<P: Into<DctTokenPayment<Env::Api>>>(
        self,
        payment: P,
    ) -> Tx<Env, From, To, MultiDctPayment<Env::Api>, Gas, Data, RH> {
        self.dct(payment)
...
where
    ...
    RH: TxResultHandler<Env>,
{
    /// Produces the normalized function call, i.e. with builtin function calls for DCT transfers.
    ///
