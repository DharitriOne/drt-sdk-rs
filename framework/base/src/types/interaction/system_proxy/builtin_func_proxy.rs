use drt_sc_codec::{Empty, TopEncode};
...
use super::builtin_func_names::{
    ...
    DCT_NFT_UPDATE_ATTRIBUTES_FUNC_NAME, SET_USERNAME_FUNC_NAME,
};
    ...
    Gas: TxGas<Env>,
{
    ...
    pub fn dct_local_burn(
        ...
        if nonce == 0 {
            return self
                .wrapped_tx
                .payment(NotPayable)
                .raw_call(DCT_LOCAL_BURN_FUNC_NAME)
                .argument(token)
                .argument(amount)
                .original_result();
        }
        ...
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call(DCT_NFT_BURN_FUNC_NAME)
            .argument(token)
            .argument(&nonce)
            .argument(amount)
            .original_result()
    ...
    pub fn dct_local_mint(
        ...
        if nonce == 0 {
            return self
                .wrapped_tx
                .payment(NotPayable)
                .raw_call(DCT_LOCAL_MINT_FUNC_NAME)
                .argument(token)
                .argument(amount)
                .original_result();
        }
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call(DCT_NFT_ADD_QUANTITY_FUNC_NAME)
            .argument(token)
            .argument(&nonce)
            .argument(amount)
            .original_result()
    ...
    pub fn nft_add_multiple_uri(
        ...
        let mut tx = self
            .wrapped_tx
            .payment(NotPayable)
            .raw_call(DCT_NFT_ADD_URI_FUNC_NAME)
            .argument(token_id)
            .argument(&nft_nonce);
    ...
    pub fn nft_update_attributes<T: TopEncode>(
        ...
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call(DCT_NFT_UPDATE_ATTRIBUTES_FUNC_NAME)
            .argument(token_id)
            .argument(&nft_nonce)
    ...
    pub fn dct_nft_create<T: TopEncode>(
        self,
        token: &TokenIdentifier<Env::Api>,
        amount: &BigUint<Env::Api>,
        ...
        let mut tx = self
            .wrapped_tx
            .payment(NotPayable)
            .raw_call(DCT_NFT_CREATE_FUNC_NAME)
            .argument(token)
            .argument(amount)
            .argument(name)
            .argument(royalties)
