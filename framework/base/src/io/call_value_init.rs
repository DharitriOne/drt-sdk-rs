use crate::{
    ...
    err_msg,
    types::{
        BigUint, RewaOrDctTokenIdentifier, DctTokenPayment, ManagedRef, ManagedType, ManagedVec,
    },
};
...
{
    CallValueWrapper::<A>::new().rewa_or_single_dct().amount
}

/// Initializes an argument annotated with `#[payment_token]`.
pub fn arg_payment_token<A>() -> RewaOrDctTokenIdentifier<A>
where
    A: CallValueApi + ManagedTypeApi,
{
