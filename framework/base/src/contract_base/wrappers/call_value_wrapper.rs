...
use crate::{
    ...
    err_msg,
    types::{
        BigUint, ConstDecimals, RewaOrDctTokenIdentifier, RewaOrDctTokenPayment,
        RewaOrMultiDctPayment, DctTokenPayment, ManagedDecimal, ManagedRef, ManagedVec,
        TokenIdentifier,
    },
};
    ...
    A: CallValueApi + ErrorApi + ManagedTypeApi,
{
    ...
    /// Will return 0 in case of an DCT transfer (cannot have both REWA and DCT transfer simultaneously).
    ...
    /// Accepts and returns either an REWA payment, or a single DCT token.
    ///
    ...
    pub fn rewa_or_single_dct(&self) -> RewaOrDctTokenPayment<A> {
        let dct_transfers = self.all_dct_transfers();
        match dct_transfers.len() {
            0 => RewaOrDctTokenPayment {
    ...
    /// Accepts and returns either an REWA payment, or a single fungible DCT token.
    ///
    /// Will halt execution if more than one DCT transfer was received, or if the received DCT is non- or semi-fungible.
    ///
    /// Works similar to `rewa_or_single_dct`,
    /// but checks the nonce to be 0 and returns a tuple of just token identifier and amount, for convenience.
    ///
    /// In case no transfer of value happen, it will return a payment of 0 REWA.
    pub fn rewa_or_single_fungible_dct(&self) -> (RewaOrDctTokenIdentifier<A>, BigUint<A>) {
        let payment = self.rewa_or_single_dct();
        if payment.token_nonce != 0 {
            A::error_api_impl().signal_error(err_msg::FUNGIBLE_TOKEN_EXPECTED_ERR_MSG.as_bytes());
        }
    ...
    pub fn any_payment(&self) -> RewaOrMultiDctPayment<A> {
        let dct_transfers = self.all_dct_transfers();
        if dct_transfers.is_empty() {
            RewaOrMultiDctPayment::Rewa(self.rewa_value().clone_value())
        } else {
            RewaOrMultiDctPayment::MultiDct(dct_transfers.clone_value())
        }
    }
}
