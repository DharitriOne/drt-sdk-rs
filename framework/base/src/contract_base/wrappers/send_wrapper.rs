...
use crate::{
    ...
    codec,
    types::{
        system_proxy, BigUint, ContractCallNoPayment, DCTSystemSCAddress,
        RewaOrDctTokenIdentifier, DctTokenPayment, FunctionCall, GasLeft, ManagedAddress,
...
/// API that groups methods that either send REWA or DCT, or that call other contracts.
    ...
    A: CallTypeApi + StorageReadApi + BlockchainApi,
{
    ...
    /// Backwards compatibility, synonymous to `dct_system_sc_tx`, which is the more appropriate name now.
    pub fn dct_system_sc_proxy(
        &self,
    ) -> system_proxy::DCTSystemSCProxyMethods<TxScEnv<A>, (), DCTSystemSCAddress, ()> {
        self.dct_system_sc_tx()
    }
    ...
    pub fn dct_system_sc_tx(
        &self,
    ) -> system_proxy::DCTSystemSCProxyMethods<TxScEnv<A>, (), DCTSystemSCAddress, ()> {
        Tx::new_tx_from_sc()
            .to(DCTSystemSCAddress)
            .typed(system_proxy::DCTSystemSCProxy)
    }
    ...
    /// Sends either REWA, DCT or NFT to the target address,
    /// depending on the token identifier and nonce
    #[inline]
    pub fn direct(
        &self,
        to: &ManagedAddress<A>,
        token: &RewaOrDctTokenIdentifier<A>,
        nonce: u64,
        amount: &BigUint<A>,
    ) {
    ...
    pub fn direct_dct(
        ...
        Tx::new_tx_from_sc()
            .to(to)
            .single_dct(token_identifier, token_nonce, amount)
            .transfer();
    }
    ...
    pub fn transfer_dct_non_zero_via_async_call(
        ...
        self.transfer_dct_via_async_call(to, token, nonce, amount)
    }
    ...
    pub fn dct_local_mint(&self, token: &TokenIdentifier<A>, nonce: u64, amount: &BigUint<A>) {
        Tx::new_tx_from_sc()
            ...
            .gas(GasLeft)
            .typed(system_proxy::UserBuiltinProxy)
            .dct_local_mint(token, nonce, amount)
            .sync_call()
    }
    ...
    pub fn dct_local_burn(&self, token: &TokenIdentifier<A>, nonce: u64, amount: &BigUint<A>) {
        Tx::new_tx_from_sc()
            ...
            .gas(GasLeft)
            .typed(system_proxy::UserBuiltinProxy)
            .dct_local_burn(token, nonce, amount)
            .sync_call()
    }
    ...
    pub fn dct_nft_create<T: codec::TopEncode>(
        ...
        Tx::new_tx_from_sc()
            ...
            .gas(GasLeft)
            .typed(system_proxy::UserBuiltinProxy)
            .dct_nft_create(token, amount, name, royalties, hash, attributes, uris)
    ...
    pub fn sell_nft(
        ...
        payment_token: &RewaOrDctTokenIdentifier<A>,
        payment_nonce: u64,
