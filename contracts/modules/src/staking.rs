...
pub trait StakingModule {
    fn init_staking_module(
        &self,
        staking_token: &RewaOrDctTokenIdentifier,
    ...
    #[endpoint]
    fn stake(&self) {
        let (payment_token, payment_amount) = self.call_value().rewa_or_single_fungible_dct();
    ...
    fn unstake(&self, unstake_amount: BigUint) {
        ...
        self.tx()
            .to(caller)
            .rewa_or_single_dct(&staking_token, 0, &unstake_amount)
            .transfer();
    }
    ...
    fn staking_token(&self) -> SingleValueMapper<RewaOrDctTokenIdentifier>;
