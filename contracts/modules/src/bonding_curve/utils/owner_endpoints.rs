...
pub trait OwnerEndpointsModule: storage::StorageModule + events::EventsModule {
            ...
            + Default,
    {
        ...
        let caller = self.blockchain().get_caller();
        let mut set_payment = RewaOrDctTokenIdentifier::rewa();

        if self.bonding_curve(&identifier).is_empty() {
            match payment_token {
                OptionalValue::Some(token) => set_payment = RewaOrDctTokenIdentifier::dct(token),
                OptionalValue::None => {
            ...
            + Default,
    {
        ...
        self.tx().to(&caller).multi_dct(tokens_to_claim).transfer();
    ...
    fn set_curve_storage<T>(
        ...
        identifier: &TokenIdentifier,
        amount: BigUint,
        payment_token_identifier: RewaOrDctTokenIdentifier,
    ) where
        T: CurveFunction<Self::Api>
            + TopEncode
            + TopDecode
            ...
            + Default,
    {
        ...
        if self.bonding_curve(identifier).is_empty() {
            ...
            payment = RewaOrDctTokenPayment::new(payment_token_identifier, 0, BigUint::zero());
