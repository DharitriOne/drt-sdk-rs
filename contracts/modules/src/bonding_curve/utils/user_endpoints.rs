...
pub trait UserEndpointsModule: storage::StorageModule + events::EventsModule {
            ...
            + Default,
    {
        ...
        self.tx()
            .to(&caller)
            .rewa_or_single_dct(&payment_token, 0u64, &calculated_price)
            .transfer();
    ...
    fn buy_token<T>(
        ...
        T: CurveFunction<Self::Api>
            ...
            + Default,
    {
        let (offered_token, payment) = self.call_value().rewa_or_single_fungible_dct();
        let payment_token =
        ...
        match requested_nonce {
            OptionalValue::Some(nonce) => {
                self.tx()
                    .to(&caller)
                    .single_dct(&requested_token, nonce, &requested_amount)
                    .transfer();
        ...
        self.tx()
            .to(&caller)
            .rewa_or_single_dct(&offered_token, 0u64, &(&payment - &calculated_price))
            .transfer();
    ...
    fn send_next_available_tokens(
        ...
        self.tx().to(caller).multi_dct(tokens_to_send).transfer();
    ...
    fn check_given_token(
        &self,
        accepted_token: &RewaOrDctTokenIdentifier,
        given_token: &RewaOrDctTokenIdentifier,
    ) {
        require!(
