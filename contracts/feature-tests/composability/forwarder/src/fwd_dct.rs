...
#[drt_sc::module]
pub trait ForwarderDctModule: fwd_storage::ForwarderStorageModule {
    ...
    fn send_dct_with_fees(&self, to: ManagedAddress, percentage_fees: BigUint) {
        ...
        self.tx()
            .to(&to)
            .single_dct(&token_id, 0, &amount_to_send)
            .transfer();
    }
    ...
    fn send_dct_twice(
        ...
        self.tx()
            .to(to)
            .single_dct(&token_id, 0, amount_first_time)
            .transfer();
        self.tx()
            .to(to)
            .single_dct(&token_id, 0, amount_second_time)
            .transfer();
    }
    ...
    fn dct_issue_callback(
        ...
        let (token_identifier, returned_tokens) = self.call_value().rewa_or_single_fungible_dct();
