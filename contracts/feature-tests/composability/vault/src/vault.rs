...
pub trait Vault {
    ...
    fn accept_funds(&self) {
        ...
        self.accept_funds_event(&self.call_value().rewa_value(), &dct_transfers_multi);
    ...
    fn accept_funds_echo_payment(
        ...
        let dct_transfers_multi = self.dct_transfers_multi();
        self.accept_funds_event(&rewa_value, &dct_transfers_multi);

        self.call_counts(ManagedBuffer::from(b"accept_funds_echo_payment"))
            .update(|c| *c += 1);

        (rewa_value.clone_value(), dct_transfers_multi).into()
    }
    ...
    fn reject_funds(&self) {
        let dct_transfers_multi = self.dct_transfers_multi();
        self.reject_funds_event(&self.call_value().rewa_value(), &dct_transfers_multi);
    ...
    fn retrieve_funds_promises(
        ...
        let return_payment =
            RewaOrDctTokenPayment::new(payment.token_identifier, payment.token_nonce, value);
    ...
    #[endpoint]
    fn retrieve_funds(&self, token: RewaOrDctTokenIdentifier, nonce: u64, amount: BigUint) {
    ...
    fn retrieve_funds_event(
        &self,
        #[indexed] token: &RewaOrDctTokenIdentifier,
        #[indexed] nonce: u64,
