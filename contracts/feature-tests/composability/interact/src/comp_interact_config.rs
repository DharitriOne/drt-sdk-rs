...
impl Config {
    ...
    pub fn token_id(&self) -> RewaOrDctTokenIdentifier<StaticApi> {
        match self.token_id.as_str() {
            "REWA" => RewaOrDctTokenIdentifier::rewa(),
            _ => RewaOrDctTokenIdentifier::dct(TokenIdentifier::from(self.token_id.as_str())),
        }
    }
