...
fn test_nft_update_attributes_and_send() {
    ...
    state.world.transfer_step(
        TransferStep::new()
            ...
            .dct_transfer(NFT_TOKEN, 1, "1"),
    );

    state
        .world
        .check_account(USER_ADDRESS)
        .dct_nft_balance_and_attributes(NFT_TOKEN_ID, 1, 1, original_attributes);
    ...
    state
        .world
        .check_account(USER_ADDRESS)
        .dct_nft_balance_and_attributes(NFT_TOKEN_ID, 1, 1, new_attributes);
}
