...
pub trait ForwarderRawAsync: super::forwarder_raw_common::ForwarderRawCommon {
    ...
    fn forward_payment(&self, to: ManagedAddress) {
        let (token, payment) = self.call_value().rewa_or_single_fungible_dct();
        self.tx()
            .to(to)
            .rewa_or_single_dct(&token, 0, &payment)
            .transfer();
    }
    ...
    fn forward_contract_call(
        &self,
        to: ManagedAddress,
        payment_token: RewaOrDctTokenIdentifier,
        ...
        (),
        ManagedAddress,
        RewaOrDctTokenPayment<Self::Api>,
        (),
    ...
    > {
        self.tx()
            ...
            .payment(RewaOrDctTokenPayment::new(
                payment_token,
                0,
                payment_amount,
            ))
    ...
    fn forward_transf_exec_dct(
        ...
        self.forward_contract_call(
            to,
            RewaOrDctTokenIdentifier::dct(token),
            payment,
            endpoint_name,
            args,
        )
