...
getFungibleDctBalance() {
    drtpy --verbose contract call ${ADDRESS} --recall-nonce --pem=${ALICE} --gas-limit=50000000 --function="getFungibleDctBalance" --arguments ${TOKEN_IDENTIFIER} --send --proxy=${PROXY} --chain=${CHAIN_ID}
    #drtpy --verbose contract query ${ADDRESS} --function="getFungibleDctBalance" --arguments ${TOKEN_IDENTIFIER} --proxy=${PROXY}
}
