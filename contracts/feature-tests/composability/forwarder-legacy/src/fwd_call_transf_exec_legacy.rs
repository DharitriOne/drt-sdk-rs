...
pub trait ForwarderTransferExecuteModule {
    ...
    fn forward_transf_exec_accept_funds(&self, to: ManagedAddress) {
        ...
        self.vault_proxy()
            .contract(to)
            .accept_funds()
            .with_rewa_or_single_dct_transfer(payment)
            .transfer_execute();
    }

    /// Tests triple as DCTTokenPayment.
    ///
    /// TODO: move somewhere else after release
    #[endpoint]
    #[payable("*")]
    fn forward_transf_exec_accept_single_dct(&self, to: ManagedAddress) {
    ...
    fn forward_transf_execu_accept_funds_with_fees(
        ...
        self.vault_proxy()
            .contract(to)
            .accept_funds()
            .with_rewa_or_single_dct_transfer((token_id, 0, amount_to_send))
            .transfer_execute();
    }
    ...
    fn forward_transf_exec_accept_funds_twice(&self, to: ManagedAddress) {
        let (token, token_nonce, payment) = self.call_value().rewa_or_single_dct().into_tuple();
        ...
        self.vault_proxy()
            .contract(to.clone())
            .accept_funds()
            .with_rewa_or_single_dct_transfer((token.clone(), token_nonce, half_payment.clone()))
        ...
        self.vault_proxy()
            .contract(to)
            .accept_funds()
            .with_rewa_or_single_dct_transfer((token, token_nonce, half_payment))
            .with_gas_limit(half_gas)
            .transfer_execute();
    }
    ...
    fn forward_transf_exec_accept_funds_return_values(
        &self,
        to: ManagedAddress,
    ) -> MultiValue4<u64, u64, BigUint, RewaOrDctTokenIdentifier> {
        let payment = self.call_value().rewa_or_single_dct();
    ...
    fn transf_exec_multi_accept_funds_v2(
        ...
        for multi_arg in token_payments.into_iter() {
            ...
            tx = tx.with_dct_transfer(payment_triple);
        }
