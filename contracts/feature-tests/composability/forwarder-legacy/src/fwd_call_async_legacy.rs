...
pub trait ForwarderAsyncCallModule {
    ...
    fn forward_async_accept_funds_half_payment(&self, to: ManagedAddress) {
        ...
        self.vault_proxy()
            .contract(to)
            .accept_funds()
            .with_rewa_or_single_dct_transfer((
    ...
    fn retrieve_funds_callback(&self) {
        let (token, nonce, payment) = self.call_value().rewa_or_single_dct().into_tuple();
    ...
    fn send_funds_twice(
        &self,
        to: &ManagedAddress,
        token_identifier: &RewaOrDctTokenIdentifier,
        amount: &BigUint,
    ) {
        self.vault_proxy()
            .contract(to.clone())
            .accept_funds()
            .with_rewa_or_single_dct_transfer((token_identifier.clone(), 0, amount.clone()))
            .async_call()
            .with_callback(
                self.callbacks()
    ...
    fn send_funds_twice_callback(
        ...
        self.vault_proxy()
            .contract(to.clone())
            .accept_funds()
            .with_rewa_or_single_dct_transfer((token_identifier.clone(), 0, cb_amount.clone()))
            .async_call()
            .call_and_exit()
    }
