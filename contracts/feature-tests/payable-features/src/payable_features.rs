...
/// i.e. the framework/Andes functionality for accepting REWA and DCT payments.
#[drt_sc::contract]
pub trait PayableFeatures {
    ...
    fn payable_any_1(
        ...
        #[payment_token] token: RewaOrDctTokenIdentifier,
    ) -> MultiValue2<BigUint, RewaOrDctTokenIdentifier> {
        (payment, token).into()
    }
    ...
    fn payable_any_2(
        ...
        let token = self.call_value().rewa_or_single_dct().token_identifier;
        (payment, token).into()
    }
    ...
    #[endpoint]
    #[payable("*")]
    fn payable_any_4(&self) -> MultiValue2<BigUint, RewaOrDctTokenIdentifier> {
        let payment = self.call_value().rewa_or_single_dct();
    ...
    #[endpoint]
    #[payable("REWA")]
    fn payable_rewa_4(&self) -> MultiValue2<BigUint, RewaOrDctTokenIdentifier> {
