...
async fn main() {
    ...
    match cmd.as_str() {
        ...
        "rewa_or_dct_payment" => state.rewa_or_dct_payment().await,
...
impl State {
    ...
    async fn rewa_or_dct_payment(&mut self) {
        let arg = RewaOrDctTokenPayment::new(
            RewaOrDctTokenIdentifier::dct(&b""[..]),
            0u64,
            BigUint::from(0u128),
        );

        let response: TypedResponse<RewaOrDctTokenIdentifier<StaticApi>> = self
            .interactor
            .sc_call_use_result(
                ScCallStep::new()
                    .call(self.contract.rewa_or_dct_payment(arg))
                    .from(&self.wallet_address)
                    .expect(TxExpect::ok().additional_error_message("SC call failed: ")),
            )
            .await;
