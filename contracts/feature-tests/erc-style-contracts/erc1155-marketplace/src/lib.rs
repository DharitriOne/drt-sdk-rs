...
pub trait Erc1155Marketplace {
    ...
    fn claim(&self) {
        ...
        for (token_identifier, amount) in claimable_funds_mapper.iter() {
            self.tx()
                .to(&caller)
                .rewa_or_single_dct(&token_identifier, 0, &amount)
                .transfer();
    ...
    fn bid(&self, type_id: BigUint, nft_id: BigUint) {
        let (payment_token, payment) = self.call_value().rewa_or_single_fungible_dct();
        require!(
        ...
        if !auction.current_winner.is_zero() {
            self.tx()
                ...
                .rewa_or_single_dct(&auction.token_identifier, 0, &auction.current_bid)
                .transfer();
        }
    ...
    fn end_auction(&self, type_id: BigUint, nft_id: BigUint) {
        ...
        if !auction.current_winner.is_zero() {
            ...
            self.tx()
                ...
                .rewa_or_single_dct(&auction.token_identifier, 0, &amount_to_send)
                .transfer();
    ...
    fn add_claimable_funds(&self, token_identifier: &RewaOrDctTokenIdentifier, amount: &BigUint) {
    ...
    fn clear_claimable_funds(&self, token_identifier: &RewaOrDctTokenIdentifier) {
        let mut mapper = self.get_claimable_funds_mapper();
    ...
    fn get_claimable_funds_mapper(&self) -> MapMapper<RewaOrDctTokenIdentifier, BigUint>;
