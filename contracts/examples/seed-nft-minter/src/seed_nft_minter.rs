    ...
    + default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn create_nft(
        ...
        let token_used_as_payment = match opt_token_used_as_payment {
            OptionalValue::Some(token) => RewaOrDctTokenIdentifier::dct(token),
            OptionalValue::None => RewaOrDctTokenIdentifier::rewa(),
        };
        require!(
    ...
    fn claim_and_distribute(&self, token_id: RewaOrDctTokenIdentifier, token_nonce: u64) {
        let total_amount = self.claim_royalties(&token_id, token_nonce);
        self.distribute_funds(&token_id, token_nonce, total_amount);
    }

    fn claim_royalties(&self, token_id: &RewaOrDctTokenIdentifier, token_nonce: u64) -> BigUint {
        ...
        for address in self.marketplaces().iter() {
            ...
            let (rewa_amount, dct_payments) = results.into_tuple();
