...
pub struct PriceTag<M: ManagedTypeApi> {
    pub token: RewaOrDctTokenIdentifier<M>,
    pub nonce: u64,
...
pub trait NftModule {
    ...
    fn buy_nft(&self, nft_nonce: u64) {
        ...
        self.tx()
            .to(ToCaller)
            .single_dct(&nft_token_id, nft_nonce, &BigUint::from(NFT_AMOUNT))
            .transfer();
    ...
    fn get_nft_price(
        &self,
        nft_nonce: u64,
    ) -> OptionalValue<MultiValue3<RewaOrDctTokenIdentifier, u64, BigUint>> {
    ...
    fn issue_callback(
        &self,
        #[call_result] result: ManagedAsyncCallResult<RewaOrDctTokenIdentifier>,
    ) {
        match result {
            ...
            ManagedAsyncCallResult::Err(_) => {
                let returned = self.call_value().rewa_or_single_dct();
    ...
    fn create_nft_with_attributes<T: TopEncode>(
        ...
        token_used_as_payment: RewaOrDctTokenIdentifier,
