...
pub trait Lottery {
    ...
    fn buy_ticket(&self, lottery_name: ManagedBuffer) {
        let (token_identifier, payment) = self.call_value().rewa_or_single_fungible_dct();
    ...
    fn distribute_prizes(&self, lottery_name: &ManagedBuffer) {
        ...
        for i in (1..total_winning_tickets).rev() {
            ...
            self.tx()
                .to(&winner_address)
                .rewa_or_single_dct(&info.token_identifier, 0, &prize)
                .transfer();
        ...
        self.tx()
            ...
            .rewa_or_single_dct(&info.token_identifier, 0, &info.prize_pool)
            .transfer();
    }
