...
fn test_raffle_and_claim() {
    ...
    for nonce in nft_nonces.into_iter() {
        let payment = DctTokenPayment::new(NFT_TOKEN_ID.into(), nonce, 1u64.into());
...

    {
        ...
        for nft_nonce in nft_nonces {
            account_setter = account_setter.dct_nft_balance(NFT_TOKEN_ID, nft_nonce, 1, ());
        }
    }
    ...
    for nonce in 1u64..=10_000u64 {
        let reward = state
            ...
            .to(REWARDS_DISTRIBUTION_ADDRESS)
            .typed(rewards_distribution_proxy::RewardsDistributionProxy)
            .compute_claimable_amount(0u64, &RewaOrDctTokenIdentifier::rewa(), 0u64, nonce)
    ...
    let mut reward_tokens: MultiValueEncoded<
        StaticApi,
        MultiValue2<RewaOrDctTokenIdentifier<StaticApi>, u64>,
    > = MultiValueEncoded::new();
    reward_tokens.push((RewaOrDctTokenIdentifier::rewa(), 0).into());
    state
        .world
        .tx()
        .from(ALICE_ADDRESS)
    ...
    for nonce in nft_nonces.iter() {
        state
            ...
            .to(REWARDS_DISTRIBUTION_ADDRESS)
            .typed(rewards_distribution_proxy::RewardsDistributionProxy)
            .was_claimed(0u64, &RewaOrDctTokenIdentifier::rewa(), 0u64, nonce)
