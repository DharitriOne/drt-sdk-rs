...
pub use drt_sc::api::ED25519_SIGNATURE_BYTE_LEN;
...
pub trait SignatureOperationsModule: storage::StorageModule + helpers::HelpersModule {
    ...
    fn withdraw(&self, address: ManagedAddress) {
        ...
        if paid_fee_token.token_identifier == RewaOrDctTokenIdentifier::rewa() {
    ...
    fn forward(
        ...
        let paid_fee = self.call_value().rewa_or_single_dct();
        let caller_address = self.blockchain().get_caller();
        ...
        new_deposit.update(|fwd_deposit| {
            require!(
                fwd_deposit.rewa_funds == BigUint::zero() && fwd_deposit.dct_funds.is_empty(),
                "key already used"
            );
            require!(
