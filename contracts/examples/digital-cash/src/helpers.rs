...
pub trait HelpersModule: storage::StorageModule {
    fn send_fee_to_address(&self, fee: &RewaOrDctTokenPayment, address: &ManagedAddress) {
        if fee.token_identifier == RewaOrDctTokenIdentifier::rewa() {
            self.tx().to(address).rewa(&fee.amount).transfer();
        } else {
            let dct_fee = fee.clone().unwrap_dct();
            self.tx()
                .to(address)
                .single_dct(&dct_fee.token_identifier, 0, &dct_fee.amount)
                .transfer();
        }
    }
    ...
    fn get_fee_for_token(&self, token: &RewaOrDctTokenIdentifier) -> BigUint {
        require!(
    ...
    fn make_fund(
        ...
        deposit_mapper.update(|deposit| {
            require!(
                deposit.rewa_funds == 0 && deposit.dct_funds.is_empty(),
                "key already used"
            );
