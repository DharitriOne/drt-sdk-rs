...
{
    #[init]
    fn init(&self, fee: BigUint, token: RewaOrDctTokenIdentifier) {
        self.whitelist_fee_token(fee, token);
    }

    #[endpoint(whitelistFeeToken)]
    #[only_owner]
    fn whitelist_fee_token(&self, fee: BigUint, token: RewaOrDctTokenIdentifier) {
    ...
    #[only_owner]
    fn blacklist_fee_token(&self, token: RewaOrDctTokenIdentifier) {
    ...
    fn claim_fees(&self) {
        ...
        for token in fee_tokens {
            ...
            if token == RewaOrDctTokenIdentifier::rewa() {
