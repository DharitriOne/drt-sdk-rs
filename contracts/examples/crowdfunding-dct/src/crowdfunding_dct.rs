...
use drt_sc::{derive_imports::*, imports::*};
pub mod crowdfunding_dct_proxy;
...
pub trait Crowdfunding {
    #[init]
    fn init(&self, target: BigUint, deadline: u64, token_identifier: RewaOrDctTokenIdentifier) {
    ...
    #[payable("*")]
    fn fund(&self) {
        let (token, _, payment) = self.call_value().rewa_or_single_dct().into_tuple();
    ...
    fn claim(&self) {
        match self.status() {
            ...
            Status::Successful => {
                ...
                self.tx()
                    .to(&caller)
                    .rewa_or_single_dct(&token_identifier, 0, &sc_balance)
                    .transfer();
            },
            Status::Failed => {
                let caller = self.blockchain().get_caller();
                ...
                if deposit > 0u32 {
                    ...
                    self.tx()
                        .to(&caller)
                        .rewa_or_single_dct(&token_identifier, 0, &deposit)
                        .transfer();
                }
            },
        }
    ...
    fn cf_token_identifier(&self) -> SingleValueMapper<RewaOrDctTokenIdentifier>;
}
